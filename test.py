...
# Function to build the sequences for LSTM
def building_data_sequences(data_X, data_Y, timesteps):
    X = []
    y = []
    for i in range(len(data_X) - timesteps + 1):
        X.append(data_X[i:(i + timesteps), :])
        y.append(data_Y[i + timesteps - 1])
    return np.array(X), np.array(y)

# Define the LSTM model
def prediction_model_002(input_shape, optimizer):
    model = Sequential([
        LSTM(2**twoexp_nodes_number_layer_1, input_shape=input_shape, return_sequences=True),
    ...
    model.compile(optimizer=optimizer, loss='mean_squared_error')  # Using standard MSE loss
    return model
...
model = prediction_model_002(input_shape, optimizer)
...
dates = df.index[-len(predictions):]
y_true = scaler_target.inverse_transform(y_val)  # Inverse transform to get actual values

# Plot the results
target_name = "Stock Price"
plot_results = generate_plot(predictions, y_true, dates, target_name)

# Display the plot results
print("Plot results:")
for plot_type, metrics in plot_results.items():
    if isinstance(metrics, dict):
        print(f"{plot_type}: {metrics}")
